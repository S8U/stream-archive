...
- **스트리밍 감지** — 등록된 채널의 방송 시작을 자동 감지 (API Polling)
- **스트리밍 녹화** — Streamlink → FFmpeg 파이프라인으로 HLS 녹화
- **실시간 시청** — 녹화 중인 방송을 HLS로 실시간 시청 가능
- **메타데이터 수집** — 채팅 (WebSocket), 시청자 수·제목·카테고리 변경 이력
...
| 구분 | 기술 |
|------|------|
| Backend | Spring Boot 3, Kotlin, JPA/Hibernate, MariaDB |
