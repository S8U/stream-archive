...
class VideoChatHistorySearchUseCase(
    ...
    fun search(uuid: String, offsetStart: Long, offsetEnd: Long): List<VideoChatHistorySearchResult> {
        ...
        // 채팅 이력 조회
        val chatHistories = videoChatHistoryRepository.findByVideoIdAndOffsetMillisGreaterThanEqualAndOffsetMillisLessThanOrderByOffsetMillisAsc(
            videoId = video.id!!,
            startOffset = offsetStart,
            endOffset = offsetEnd
        )

        return chatHistories.map { VideoChatHistorySearchResult.from(it) }
    }
