...
class TwitchApiClient(
    ...
    /**
     * 401 에러 발생 시 OAuth 토큰 재발급 후 [block]을 재시도한다.
     */
    private fun <T> executeWithRetry(block: () -> T?): T? {
        return try {
            ...
            if (e.statusCode.value() == 401) {
                logger.warn("TwitchApiClient: Twitch API 401 Unauthorized - refreshing OAuth token")
                refreshOauthToken()
                block()
            } else {
                logger.error("TwitchApiClient: Twitch API call failed")
